"""BusinessBroker.net listing scraper (WI / IL)."""
...
class BusinessBrokerScraper(BaseScraper):
    SOURCE_NAME = "businessbroker"
    BASE_URLS = [
        "https://www.businessbroker.net/businesses-for-sale/wisconsin/",
        "https://www.businessbroker.net/businesses-for-sale/illinois/",
    ]
    ...
    async def scrape_detail(self, url: str) -> dict:
        ...
        desc_el = soup.select_one(".listing-description, [class*='description']")
        description = desc_el.get_text(" ", strip=True) if desc_el else None
        text = soup.get_text(" ", strip=True)
        return {
            "description": description,
            "asking_price": self.parse_money(self._label_value(soup, text, "Asking Price")),
            "annual_revenue": self.parse_money(self._label_value(soup, text, "Gross Revenue"))
            or self.parse_money(self._label_value(soup, text, "Sales")),
    ...
    def parse_listing(self, html: str, page_url: str) -> list[dict]:
        ...
        for a in soup.select("a[href]"):
            href = a.get("href") or ""
            if "/listing/" not in href.lower() and "/business-for-sale/" not in href.lower():
                continue
            ...
            if parsed.netloc and "businessbroker" not in parsed.netloc.lower():
                continue
